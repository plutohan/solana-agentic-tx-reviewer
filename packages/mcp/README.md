...
## Tools
- **`review_transaction`** — provide a confirmed `signature` or an unsigned base64 `rawTransaction` (+ optional `cluster`). Returns the risk level (info/low/medium/high), a 0-100 score, the findings, and a plain-English summary. Calls the reviewer API (configurable). Use this before signing.
...
Then an agent can call `review_transaction` before it signs, and gate on `risk.level === "high"`.
