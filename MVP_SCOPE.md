...
| AI status diagnostic | `GET /api/ai-status` reports which AI config is present (booleans only, no secrets). `?test=1` does a minimal live ping to the configured provider and reports ok/status/error. | `src/app/api/ai-status/route.ts` |
