...
- **Public API and SDK** (`src/app/api/v1/`, `sdk/`): a stable, versioned, CORS-enabled `POST /api/v1/review` (best-effort per-IP rate limiting, optional `x-api-key`, no client RPC override), an OpenAPI 3.1 spec at `/api/v1/openapi.json`, and a dependency-free typed TypeScript SDK (`solana-tx-reviewer-sdk`) with `reviewSignature()` / `reviewUnsigned()`, so any wallet or agent gets a typed `ReviewResult` in one call before it signs. Reference docs in `docs/API.md`, examples in `examples/`. Verified end to end against the running endpoint.
...
This is the most strategic step and the clearest expression of the agent-loop framing. The reviewer should be the check other software calls before it signs. The core of this milestone is now built and deployed.

**Shipped**
- A stable, versioned, CORS-enabled public endpoint: `POST /api/v1/review`, with best-effort per-IP rate limiting (rate-limit headers + `Retry-After`), an optional `x-api-key` gate, and no client RPC override (the SSRF surface is closed for public callers). See `docs/API.md`.
- A machine-readable OpenAPI 3.1 spec at `GET /api/v1/openapi.json`.
- A dependency-free TypeScript SDK in `sdk/` (`solana-tx-reviewer-sdk`): `reviewSignature()`, `reviewUnsigned()`, `review()`, fully typed `ReviewResult`, timeout/abort, and typed `ReviewerError`. Works in Node 18+ and browsers.
- Integration examples in `examples/`: a Node script and a reference wallet pre-sign hook.

**Remaining follow-ups**
- Move rate limiting to a shared store (Upstash/Redis) so limits hold across serverless instances.
- Publish the SDK to npm and wire a real wallet/extension integration end to end.

**Acceptance criteria**
- A third-party wallet or agent gets a `ReviewResult` for a signature or an unsigned transaction with one SDK call. (Met: verified end to end against the running endpoint.)
- The API is versioned and rate-limited, and nothing it exposes ever signs or sends a transaction. (Met.)
