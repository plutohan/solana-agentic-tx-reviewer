...
export function buildPrompt(tx: ParsedTransaction, risk: RiskReport): string {
  return [
    ...
    "",
    `Mode: ${tx.simulated ? "SIMULATION of an UNSIGNED transaction (pre-sign; not yet executed)" : "confirmed transaction"}`,
...
function placeholderExplanation(
  ...
  const bullets: string[] = [
    tx.simulated
      ? `Simulation: ${tx.success ? "would succeed" : "would FAIL"} · ${(tx.computeUnitsConsumed ?? 0).toLocaleString()} compute units`
  ...
  for (const c of tx.tokenBalanceChanges.slice(0, 5)) {
    const token = c.symbol ?? `mint ${shortPubkey(c.mint)}`;
    bullets.push(
      `${c.delta >= 0 ? "+" : ""}${formatTokenAmount(c.delta, c.decimals)} ${token} · ${shortPubkey(c.owner ?? c.account)}`,
    );
  }
  ...
  const opening = tx.simulated
    ? `This is a read-only simulation of an unsigned transaction. If signed and sent now, it ${status} on ${tx.cluster}. ` +
      `It requires ${tx.signers.length} signer(s) and interacts with ${tx.programsInvoked.length} program(s): ${programNames}. `
  ...
  const summary = opening + tokenSentence + riskSentence;
  ...
  return {
    ...
    caveats: [
      ...(tx.simulated
        ? [
            "This is a simulation with a replaced blockhash. The real result after signing can differ if on-chain state changes before you submit.",
          ]
        : []),
